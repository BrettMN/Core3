...
ExperienceConverter = ScreenPlay:new {
	xpConversion = {
		combat = {
			{ "bountyhunter",                    1 },
			{ "combat_general",                  1 },
			{ "combat_meleespecialize_onehand",  1 },
			{ "combat_meleespecialize_polearm",  1 },
			{ "combat_meleespecialize_twohand",  1 },
			{ "combat_meleespecialize_unarmed",  1 },
			{ "combat_rangedspecialize_carbine", 1 },
			{ "combat_rangedspecialize_heavy",   1 },
			{ "combat_rangedspecialize_pistol",  1 },
			{ "combat_rangedspecialize_rifle",   1 },
			{ "squadleader",                     1 }
		},
		senses = {
...
function ExperienceConverter:sendConversionSUI(pPlayer, pNpc, experienceType)
	...
	if (conversionList == nil) then
		printLuaError("ExperienceConverter:sendConversionSUI, could not grab xp conversion table for type " ..
		experienceType)
		return
	end
...
function ExperienceConverter:convertXpTypeCallback(pPlayer, pSui, eventIndex, args)
	...
	sui.setTitle("@quest/force_sensitive/utils:xp_transfer_prompt")
	sui.setPrompt("How much " ..
	getStringId("@exp_n:" .. chosenXp) .. " experience do you wish to convert?\n(1 for " .. ratio .. " conversion ratio)")
...
function ExperienceConverter:convertXpTransferCallback(pPlayer, pSui, eventIndex, transferInputFromValue,
													   transferInputToValue)
	local playerID = SceneObject(pPlayer):getObjectID()
