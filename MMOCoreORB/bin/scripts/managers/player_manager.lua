...
-- Entries start at 1, not 0: every reward below is milestone 1, and the reward
-- list filter skips any reward whose milestone exceeds the one being claimed, so
...
-- instead of waiting a day between each. The count of immediate rewards is the
-- number of entries in veteranRewardMilestones, so add or remove entries to tune
-- it. Set to 0 for stock behaviour, where each milestone is a day of account age.
-- Never set veteranRewardAdditionalMilestones to 0: the additional-milestones
-- loops step by that value and would never terminate.
veteranRewardsIgnoreAccountAge = 1
